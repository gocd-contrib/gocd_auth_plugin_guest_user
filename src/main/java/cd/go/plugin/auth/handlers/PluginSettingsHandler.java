...
import cd.go.plugin.auth.util.MapBuilder;
...
import static cd.go.plugin.auth.util.Json.*;
import static cd.go.plugin.auth.util.MapBuilder.create;
import static cd.go.plugin.auth.util.Resources.get;
    ...
    public static Handler validateConfiguration() {
        return new Handler() {
            ...
            public GoPluginApiResponse call(GoPluginApiRequest request) {
                ...
                if (isEmpty(settings, SETTINGS_USERNAME_KEY)) addValidationError(validationResponse, SETTINGS_USERNAME_KEY, "Username");
                if (isEmpty(settings, SETTINGS_USER_DISPLAY_NAME_KEY)) addValidationError(validationResponse, SETTINGS_USER_DISPLAY_NAME_KEY, "Display name");
                if (isEmpty(settings, SETTINGS_USER_EMAIL_KEY)) addValidationError(validationResponse, SETTINGS_USER_EMAIL_KEY, "Email address");
